...
@WebServlet(name = "AdminUpdateStatusServlet", urlPatterns = {"/admin/profile/status/*"})
public class AdminUpdateStatusServlet extends HttpServlet {
    ...
    private void updateUserStatus(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(request.getParameter("to").equals("ADMIN")) {
            new UserFactory().updateRole(user, true);
            ResponseHelper.pushSessionCode(ResponseHelper.MAKE_ADMIN_COMPLETE);
        } else if(request.getParameter("to").equals("USER")) {
            user = new UserFactory().updateRole(user, false);
            ResponseHelper.pushSessionCode(ResponseHelper.REMOVE_ADMIN_COMPLETE);
        ...
        response.sendRedirect(RouteHelper.generateURL((String) request.getSession().getAttribute("profile.view.current.path")));
    }

    private User getUserByPath(HttpServletRequest request) {
        User user = null;

        if (request.getRequestURI().endsWith("admin/profile/status") || request.getRequestURI().endsWith("admin/profile/status/")) {
            ...
            if (splits[splits.length - 4].equals("admin") && splits[splits.length - 3].equals("profile") && splits[splits.length - 2].equals("status")) {
                try {
                    user = new UserFactory().find(Integer.parseInt(splits[splits.length - 1].split("\\?")[0]));
                } catch (Exception ex) {
                    try {
                        user = new UserFactory().findByUsername(splits[splits.length - 1].split("\\?")[0]);
