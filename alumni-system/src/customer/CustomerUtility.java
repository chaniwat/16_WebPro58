...
public class CustomerUtility {
    ...
    public static int addCustomer(Customer customer) {
        int customerid = 0;
        try {
            String sql = "INSERT INTO customerApp VALUES(0, ?, ?, ?, ?, ?, ?)";
            ...
            sql = "SELECT id FROM customerApp WHERE firstname = ? AND lastname = ?";
    ...
    public static Customer getCustomer(int customerId) {
        try {
            String sql = "SELECT * FROM customerApp WHERE id = ?";
            PreparedStatement stmt = Database.getInstance().getConnection().prepareStatement(sql);
    ...
    public static Customer getCustomer(String firstname, String lastname) {
        try {
            String sql = "SELECT * FROM customerApp WHERE firstname = ? AND lastname = ?";
            PreparedStatement stmt = Database.getInstance().getConnection().prepareStatement(sql);
